 ...
 * 备注：
 */
@Service("nptDssOracleManager")
@Transactional(propagation = Propagation.REQUIRED)
public class NptOracleManager implements NptDatabaseManager,PublicBean {
    ...
    public String[] makeDistinctSql(String dtName, NptLogicDataField field, String where) {
        ...
        result[1] = "select distinct " + field.getFieldDbName() + " from " + dtName + " where " + field.getFieldDbName() + " is not null ";
        if(!StringUtils.isEmpty(where)){
            result[1] += " and " + where;
        }

        result[0] = "select count(1) from (" + result[1] + ") nptTB";
    ...
    private String makeConditionString(Map<String, String> where, Collection<NptLogicDataField> fields,Boolean exact) {
        ...
        if (null != where && !where.isEmpty()) {
            ...
            for (String key : keys) {
                ...
                if (true == exact) {
                    sb.append("replace("+key+",' ','')").append(" = '").append(noBlankSearchValue).append("'");
                } else{
                    sb.append(noBlankColumnName).append(" LIKE '%").append(noBlankSearchValue).append("%'");
                }
                if (idx < keys.size()) {
