<#include "./common.ftl">
<link href="${wctx}/pub/CreditStyle/pagination/pagination.css" rel="stylesheet">
<script src="${wctx}/pub/CreditStyle/pagination/jquery.pagination.js"></script>
<script>
...
//			async: false,
			...
			success: function(result,code,dd) {
                ...
                $("table#pageData tbody").html(result);
                $("div#pageData").html(result);
				pagination.initPagination(pagination.url, pagination.param, pagination.callback);
                $(".loadDiv").hide();
//				waitting.close();
			},
			error:function(){
