...
<div class="wrap">
    ...
    <#include "include/header.ftl"/>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>“信用柳州”官方网站（以下简称“本网站”）为本网站用户提供企业、重点人群和企业、个人自身在柳州区域内的信用信息在线查询服务。为明确用户在接受和提供本网站服务过程中的权利和义务，特制订本协议。用户在通过本网站查询信用信息前，请仔细阅读本协议的全部条款。一经点击 “同意”，则协议成立。如果您是本网站的日常用户，请随时关注本协议以及网站内容的更新。本着平等自愿的原则，双方就相关事宜达成如下协议:</p>
                        <p>第一条：服务事项</p>
                        <p>本网站提供柳州地区企业、重点人群和企业、个人自身信用信息在线查询服务。</p>
                        <p>第二条：用户的权利和义务</p>
        ...
        <div class="modal fade" id="myModal_authed" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
            <div class="modal-dialog" role="document" style="top:300px;">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                        ...
                        <div id="mainContent">
                            <div class="query-con">
                                    ...
                                    <div class="banner-con">
                                        ...
                                        <div class="banner-input">
                                            <span class="box-input">
                                                <input id="queryInput" type="text" onfocus="clearMsg(this)" placeholder="请输入企业名称，法定代表人姓名，工商注册号，组织机构代码">
                                                <div id="info-tip" class="index-search-tip">
                                                    <p class="search-count " id="indexTip">搜索关键词不得少于２个字符</p>
                                                    <span class="corner-main">◆</span>
                                                    <span class="corner-bottom">◆</span>
                                                </div>
                                            </span>
                                            <button id="serachInputModal" class="btn"  data-toggle="modal" onclick="searchInput()"></button>
                                ...
                                <div class="modal fade" id="codeModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                                    <div class="code-modal modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                            ...
                                            <div class="modal-body">
                                                <#--<div class="col-md-2 control-label left_text">验证码<font>*</font></div>-->
                                                    <div class="" id="showcode">
                                                        <div class="">
                                                            <input type="text" placeholder="请输入验证码" autocomplete="false" class="form-control pull-left yz_input bus-code"
                                                                   maxlength="6" name="captcha" id="captcha"  onfocus="clearMsg(this)">
                                                            <div id="code-tip" class="index-search-tip">
                                                                <p class="search-count " id="codeTip">请输入验证码</p>
                                                                <span class="corner-main">◆</span>
                                                                <span class="corner-bottom">◆</span>
                                                            </div>
                                                            <div id="modalCode" class="pull-left yzm_img m-l-sm">

                                                                <#--<img id="search-idxCaptcha" class="idxCaptcha" onclick="this.src='/captcha.svl?d='+new Date()" alt="" src="/captcha.svl">-->
                                                            </div>
                                                        </div>
                                                    </div>
                                            </div>
                                            <div class="modal-footer" style="text-align: center;">
                                                <#--<span style="color:red;">提示：可以通过点击输入框的外部和内部，来切换验证码！</span>-->
                                                <button id="codeModalBtn" style="width:100px;" type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                                                <button  type="button" class="btn btn-primary" onClick="searchInfo();" style="width:100px;">确认</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
//            console.log($(this).attr("id"));
    ...
    //    获取验证码
//    var searchHtml=$("#search-idxCaptcha");
    $("#showcode .bus-code").focus(function(){
        idxCaptchaHtml.attr("id","search-idxCaptcha");
        $(".idxCaptcha").remove();
        $("#modalCode").append(idxCaptchaHtml);
        $("#search-idxCaptcha").click();
    });
    function searchInput(){
        var keyWord = $('#queryInput').val();
        if(keyWord.length< 2) {
            $("#serachInputModal").removeAttr("data-target","#codeModal" );
            $("#info-tip").css("display", "block");
        }
        else{
            $("#serachInputModal").attr("data-target","#codeModal" )
        }
    }
    function searchInfo() {
        var captValue=$("#captcha").val();
        var id = $('.banner-title a[class=banner-on]').parent().attr('id');
        var keyWord = $('#queryInput').val();
        if (keyWord && keyWord.length > 1 && captValue.length > 0) {
            $(".load_main").show();
            if (id == "bus") {

                $.ajax({
                    url: encodeURI("/credit/query/validate.do"),
                    data:{
                        keyword:keyWord,
                        captcha:captValue
                    },
                    success: function (data) {
                        if(0 == data.code) {
                            $("#codeModalBtn").click();
                            $.ajax({
                                url: encodeURI("/credit/query/bs/bsSearch.do"),
...
//        $(".index-search-tip").css("display", "none");
        $(e).siblings(".index-search-tip").css("display", "none");
    }
//    模态框同意协议
    function agree() {
