...
import com.npt.querier.impl.NptCreditSpaqQuerierBase;
 ...
 */
@Controller
@RequestMapping(value = "/pub/spaq")
public class NptCreditSpaqController {

    @Autowired
    private NptCreditSpaqQuerierBase querier;
    ...
    public String list(NptWebBridgeBean bean, int flag, HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
        ...
        NptDict gCode;
        if (flag == NptDict.BMHG_SPAQ_AQDJ.getCode()) {
            gCode = NptDict.BMHG_SPAQ_AQDJ;
        } else if (flag == NptDict.BMHG_SPAQ_QYHMD.getCode()) {
            gCode = NptDict.BMHG_SPAQ_QYHMD;
        } else {
            gCode = NptDict.BMHG_SPAQ_ZRXKZ;
        }
