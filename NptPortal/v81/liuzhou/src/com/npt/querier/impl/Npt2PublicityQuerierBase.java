...
public class Npt2PublicityQuerierBase extends NptAbstractCommonQuerier implements Npt2PublicityQuerier {
    ...
    /**
     *  author: zhanglei
     *  date:   2017/06/16 14:14
     *  note:
     *
     */
    @Override
    public NptBaseModel getThisModel(String type) {
        NptBaseModel model;
        if (type.equals("ap")) {
            model = getService().lookupModelByCategoryAndHost(NptDict.BMC_OUTSIDE, NptDict.BMH_2PUBAP);
        } else {
            model = getService().lookupModelByCategoryAndHost(NptDict.BMC_OUTSIDE, NptDict.BMH_2PUBAL);
        }
        return model;
    }
}
