...
public class NptCreditServiceIndexController {
    ...
    public void swfUpload(
        ...
        ResponseUtils.renderText(response, fileName);
    }
    ...
    /**
     * 作者: 张磊
     * 日期: 2017/05/03 上午10:31
     * 备注: 承诺申请
     */
    @RequestMapping(value = "/promise/add.do", method = RequestMethod.GET)
    public String addPromise(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
        CmsUser user = CmsUtils.getUser(request);
        if (user != null) {
            CmsUserExt ext = cmsUserExtMng.findById(user.getId());
            modelMap.addAttribute("userExt", ext);
            ...
            if (ext.getFlag().equals("person")) {
                request.getSession().setAttribute("realNameLabel", "个人名称");
                request.getSession().setAttribute("idCardLabel", "身份证号");
            } else {
                request.getSession().setAttribute("realNameLabel", "企业名称");
                request.getSession().setAttribute("idCardLabel", "统一信用代码");
            }
        }
        modelMap.addAttribute("user", user);
        return NPTCreditFTLBox.FTL_CREDIT_SERVICE_PROMISE_ADD;
    }

    /**
     * 作者: 张磊
     * 日期: 2017/05/03 上午10:35
     * 备注: 申请记录
     */
    @RequestMapping(value = "/promise/list.do", method = RequestMethod.GET)
    public String listPromise(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
        CmsUser user = CmsUtils.getUser(request);
        if (user != null) {
            CmsUserExt ext = cmsUserExtMng.findById(user.getId());
            modelMap.put("userExt", ext);
        ...
        modelMap.put("user", user);
        return NPTCreditFTLBox.FTL_CREDIT_SERVICE_PROMISE_LIST;
    }
}
