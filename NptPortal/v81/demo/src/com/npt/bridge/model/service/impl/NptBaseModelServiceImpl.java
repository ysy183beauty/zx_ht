...
public class NptBaseModelServiceImpl implements NptBaseModelService {
    ...
    public NptDict getPaginationData(NptLogicDataType type, Collection<NptLogicDataField> fields, NptWebBridgeBean bean, Boolean applyed,Boolean exact) {
        ...
        if(null != fields && fields.size() > 0 && null != type) {
            ...
            if(totalCount > 0){
                ...
                List<String> titles = fields.stream().map(p -> p.getFieldName()).collect(Collectors.toList());
    ...
    public NptDict loadBaseModelAuthGroupsByPK(NptWebBridgeBean bean, NptBaseModel m,NptDict lockLevel,Boolean applyed) {
        ...
        if(null == groupList || groupList.isEmpty()){
            ...
            Collection<NptBaseModelPool> aPoolList = getBaseModelGrouPools(m,null);
            if(null != aPoolList && !aPoolList.isEmpty()){

                NptWebDetailGroup wGroup = loadOrderedGroupoolsData(
                        m.getModelNote(),
                        aPoolList,
                        pkValue,
                        lockLevel,
            ...
            for(NptBaseModelGroup group:groupList) {
                ...
                if(null == listGroupId || (null != listGroupId && listGroupId.equals(group.getId()))) {
                    Collection<NptBaseModelPool> aPoolList = getBaseModelGrouPools(group,null);
                    if (null != aPoolList && !aPoolList.isEmpty()) {
                        NptWebDetailGroup wGroup = loadOrderedGroupoolsData(
                                group.getGroupTitle(),
                                aPoolList,
        ...
        if(null != mainPool){
            ...
            if(null != dataType && null != pkField){
                ...
                if(NptCommonUtil.INTEGER_1.equals(count)) {
                    ...
                    return NptDict.RST_EXCEPTION("系统暂未收录您的信用数据");
                }
            }
        }
        ...
        if(null != poolList && !poolList.isEmpty()){
            ...
            while (ite.hasNext()){
                ...
                if(null == providerId || (null != providerId && providerId.equals(poolProvider.getId()))) {
                    NptWebDetailBlock block = getBaseModelGrouPoolData(pool, String.valueOf(pkValue),lockLevel, false,applyed);
     ...
     */
    @Override
    public NptWebDetailBlock getBaseModelGrouPoolData(NptBaseModelPool pool, String pkValue,NptDict lockLevel, Boolean first,Boolean applyed) {
        //构建结果块
        ...
        if(null != pkValue && !pkValue.isEmpty()){
            ...
            //构建查询语句
            String[] sql = dataBaseService.makeListDataSql(
            ...
            int totalCount = dataBaseService.getCount(sql[0]);
            if(totalCount > 0){
                if(null == lockLevel || pool.getLockLevel() <= lockLevel.getCode()) {
    ...
    @Override
    public NptWebDetailBlock getBaseModelGrouPoolData(Long poolId, String pkValue, NptDict lockLevel, Boolean first, Boolean applyed) {
        return getBaseModelGrouPoolData(poolManager.fastFindById(poolId), pkValue,lockLevel, first,applyed);
    }
