...
pagination.initPagination(ctx+"/npt/grs/query/business/reportIndexAjax.action", {webInvokeCondition: catchObj()}, showSecurityFunction);
function search() {
    $.ajax({
        type: "post",
        url: "reportIndexAjax.action",
...
function showDetail(data) {
    ...
    $.ajax({
        url: ctx+"/npt/grs/query/business/authIndex.action",
...
function loadApplyColumns(pid, pvalue) {
    ...
    $.ajax({
        url: ctx+"/npt/grs/query/business/authLoadApplyFields.action",
        data: {webInvokeCondition: JSON.stringify(dataObject)},
        timeout: 30000,
        beforeSend: function () {
            $(".loadDiv").show();
...
function showMore(datasourceId, keyValue) {
    ...
    $.ajax({
        type: "post",
        url: ctx+"/npt/grs/query/business/authBlockMore.action",
        data: {webInvokeCondition: JSON.stringify(dataObject)},
        timeout: 30000,
        beforeSend: function () {
            $(".loadDiv").show();
...
function jumpMore(datasourceId, keyValue) {
    ...
    $.ajax({
        type: "post",
        url: ctx+"/npt/grs/query/business/getGroupoolLinkedPoolData.action",
        data: {webInvokeCondition: JSON.stringify(dataObject)},
        timeout: 30000,
        beforeSend: function () {
            $(".loadDiv").show();
...
function loadPoolData(datasourceId, keyValue, obj) {
    ...
    $.ajax({
        type: "post",
        url: ctx+"/npt/grs/query/business/authBlockLasted.action",
        data: {webInvokeCondition: JSON.stringify(dataObject)},
        timeout: 30000,
        beforeSend: function () {
//            $(".loadDiv").show();
...
function baseFilter(size,primarykeyValue) {
    ...
    $.post(ctx+"/npt/grs/query/business/authIndexAjax.action" ,{webInvokeCondition: JSON.stringify(data)},function (data) {
...
function showReport() {
    ...
    $.post(ctx+"/findreport/getReportById.action", {id: $("#template").val()}, function (data) {
        // if(data.result){
        var url=ctx+"/ReportServer?id=" + $("#template").val() + "&reportlet=" + data.message + "&rptype=0&businessId="+businessId;
        console.log(url);
 ...
 * 历史报告
 */
function loadReportHistory(rpbs,rpmc) {
    $("#detailDiv").empty();
    $("#logDiv").empty();
    $.ajax({
        ...
        success: function (data) {
            ...
            $.post(ctx+"/npt/web/finereport/log/list.action",{rptype: 0,rpbs: rpbs,rpmc: rpmc},function (data) {
